...
export default function RegisterPage() {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    if (result?.error) {
      const isInfo = result.error.startsWith("Account created!");
      if (isInfo) {
      ...
      >
        ...
        <motion.div variants={fadeUp} className="mb-8">
          ...
          <p className="text-sm text-[--on-surface-variant] font-[family-name:var(--font-inter)]">
            Start your flow state journey. Free forever.
          </p>
        </motion.div>
        ...
        >
          <motion.div variants={fadeUp}>
            ...
            <motion.input
              ...
              animate={{
                ...
                boxShadow:
                  focused === "name"
                    ? "0 0 0 3px rgba(192,193,255,0.08), inset 0 1px 0 rgba(255,255,255,0.05)"
                    : "none",
              }}
              transition={{ duration: 0.2 }}
              className="w-full px-3.5 py-3 rounded-xl bg-white/5 text-[--on-surface] text-sm placeholder:text-[--outline] font-[family-name:var(--font-inter)] focus:outline-none transition-colors"
              style={{ border: "1px solid rgba(255,255,255,0.1)" }}
            />
          </motion.div>
          ...
          {info && (
            <motion.p
              initial={{ opacity: 0, y: -8 }}
              animate={{ opacity: 1, y: 0 }}
              className="text-xs text-[--success] font-[family-name:var(--font-inter)] px-1"
            >
              {info}
            </motion.p>
          )}
          ...
          <motion.div variants={fadeUp}>
            <motion.button
              ...
              style={{
                boxShadow:
                  "inset 0 1px 0 rgba(255,255,255,0.2), 0 6px 24px rgba(99,102,241,0.4)",
              }}
            >
              {loading ? (
                <>
